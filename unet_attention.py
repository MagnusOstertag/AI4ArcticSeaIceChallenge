...
class ExpandingBlock(torch.nn.Module):
    ...
    def __init__(self, options, input_n, output_n):
        ...
        # Attenttion
        self.attention = AttentionBlock(F_g=output_n, F_l=output_n, n_coefficients=int(output_n/2))
...
class AttentionBlock(torch.nn.Module):
    ...
    def __init__(self, F_g, F_l, n_coefficients):
        ...
        self.W_gate = torch.nn.Sequential(
            torch.nn.Conv2d(F_g, n_coefficients, kernel_size=1, stride=1, padding=0, bias=True),
            torch.nn.BatchNorm2d(n_coefficients)
        )

        self.W_x = torch.nn.Sequential(
            torch.nn.Conv2d(F_l, n_coefficients, kernel_size=1, stride=1, padding=0, bias=True),
            torch.nn.BatchNorm2d(n_coefficients)
        )

        self.psi = torch.nn.Sequential(
            torch.nn.Conv2d(n_coefficients, 1, kernel_size=1, stride=1, padding=0, bias=True),
